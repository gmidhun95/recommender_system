    ...
    for item,rating in items:
        ...
        for l in temp:
            ...
            if order>=k_values:
                break
        if den!=0.0:
            predicted_values.append((item,num*1.0/den))
        # predicted_values.append((item,num*1.0/den))
...
pickle.dump(predictions,open('final_predictions_baseline.dat','wb'))